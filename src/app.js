...
function renderProgress() {
  ...
  app.innerHTML = `<section class="progress-hero"><div><span class="eyebrow">Meta · ${formatDate(settings().targetDate)}</span><h1>Listo para buscar trabajo</h1><p>${checklist.filter((value) => value.done).length} de ${checklist.length} competencias demostradas.</p></div><strong>${ready}%</strong></section><section class="metric-grid">${metric("Progreso general", `${Math.round(total.mastered / Math.max(1, total.total) * 100)}%`)}${metric("Precisión histórica", `${total.accuracy}%`)}${metric("Tiempo estudiado", formatMinutes(time))}${metric("Racha", `${studyStreak()} días`)}${metric("Dominados", total.mastered)}${metric("Errores pendientes", storageService.getErrors({ pendingOnly: true }).length)}</section><section class="transfer-cta panel"><div><span class="eyebrow">Respaldo portátil</span><h2>Lleva tu avance a otro dispositivo</h2><p>Descarga un archivo y luego impórtalo en el navegador donde quieras continuar.</p></div><button class="primary" data-route="/backup">Exportar o importar avance</button></section><div class="dashboard-grid"><section class="panel"><h2>Progreso por nivel</h2>${state.curriculum.levels.map((level) => { const stats = aggregate(state.allItems.filter((item) => item.level === level.id)); const percent = Math.round(stats.mastered / Math.max(1, stats.total) * 100); return `<div class="progress-level"><span class="level-badge ${levelClass(level.id)}">${level.id}</span><span><b>${esc(level.title)}</b><small>${stats.mastered}/${stats.total} dominados · ${stats.learning} aprendiendo</small></span>${bar(percent)}<strong>${percent}%</strong></div>` }).join("")}<h2>Rendimiento por modalidad</h2>${Object.entries(modeStats).sort((a,b)=>b[1].attempts-a[1].attempts).slice(0,10).map(([mode,value])=>`<div class="performance-row"><span>${esc(MODE_LABELS[mode]||mode)}</span>${bar(value.correct/value.attempts*100)}<b>${Math.round(value.correct/value.attempts*100)}%</b></div>`).join("") || "<p class='muted'>Completa una sesión para ver el desglose.</p>"}</section><aside class="panel"><h2>Competencias laborales</h2><div class="readiness-list">${checklist.map((value) => `<div class="${value.done ? "done" : ""}"><span>${value.done ? "✓" : "○"}</span><p><b>${esc(value.label)}</b>${bar(value.progress)}</p><em>${value.progress}%</em></div>`).join("")}</div></aside></div>${privacy}`;
}

function renderBackup(message = "", tone = "") {
  setHeader("Transferir avance", "Exportar e importar localStorage");
  const data = storageService.exportData();
  const summary = data.summary;
  app.innerHTML = `<section class="page-intro compact"><span class="eyebrow">Respaldo local</span><h1>Continúa exactamente donde te quedaste</h1><p>El archivo contiene tu progreso, configuración, errores, listas, sesiones y la sesión en curso. No contiene el banco de ejercicios.</p></section>${message ? `<div class="notice ${tone}">${esc(message)}</div>` : ""}<section class="transfer-grid"><article class="panel transfer-card"><span class="transfer-icon">↓</span><h2>Exportar avance</h2><p>Descarga un JSON privado que puedes guardar o mover a otro dispositivo.</p><div class="backup-summary">${metric("Elementos con avance", summary.itemsWithProgress)}${metric("Dominados", summary.masteredItems)}${metric("Errores", summary.errors)}${metric("Sesiones", summary.sessions)}</div><button class="primary full" data-action="export-data">Descargar mi avance</button></article><article class="panel transfer-card"><span class="transfer-icon">↑</span><h2>Importar avance</h2><p>Selecciona un respaldo de English Drill. Se reemplazará el avance de este navegador y antes se conservará una copia preventiva local.</p><label class="primary full import-label" for="importProgressFile">Seleccionar respaldo JSON</label><input id="importProgressFile" type="file" accept="application/json,.json" hidden><small>Máximo 20 MB · El archivo se procesa únicamente en este dispositivo.</small></article></section><section class="panel transfer-steps"><h2>Cómo mover tu avance</h2><ol><li>En el dispositivo actual, pulsa <b>Descargar mi avance</b>.</li><li>Envía o copia el archivo JSON al otro dispositivo.</li><li>Abre English Drill allí y elige <b>Seleccionar respaldo JSON</b>.</li><li>Al terminar, el progreso se verá de inmediato y quedará guardado en el almacenamiento local del navegador.</li></ol></section>${privacy}`;
}
...
function exportData() {
  const blob = new Blob([JSON.stringify(storageService.exportData(), null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `english-drill-avance-${new Date().toISOString().slice(0, 10)}.json`; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000);
}

async function importData(file) {
  const renderImportMessage = (message, tone = "") => routePath() === "/backup" ? renderBackup(message, tone) : renderSettings(message);
  try {
    if (!file || file.size > 20 * 1024 * 1024) throw new Error("El respaldo supera el máximo permitido de 20 MB.");
    const result = storageService.importData(JSON.parse(await file.text()));
    storageService.seedInitialProgress(state.allItems);
    state.selection = new Set(settings().selectedItemIds || []);
    state.context = state.setup = state.active = state.session = state.question = state.results = null;
    applyTheme(settings().theme);
    renderImportMessage(`Avance importado: ${result.itemsWithProgress} elementos, ${result.masteredItems} dominados, ${result.errors} errores y ${result.sessions} sesiones. Ya quedó guardado en este navegador.`, "success");
  } catch (error) { renderImportMessage(error.message, "error") }
}
...
document.addEventListener("change", (event) => {
  ...
  if (event.target.id === "importProgressFile" && event.target.files[0]) importData(event.target.files[0]);
});
